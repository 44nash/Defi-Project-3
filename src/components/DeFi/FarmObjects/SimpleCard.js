...
import {
  ...
  swapWithInput,
  swapAndLiquidity,
  swapAndLiquidityWithInput,
  swapLiquidityFarmWithInput,
} from '../uniswap';
...
class SimpleCard extends React.Component {
  ...
  render() {
    ...
    return (
      <div style={cardStyle}>
        <p style={{ color: 'dodgerblue', fontSize: '20px' }}>
          {this.props.path}
        </p>
        <p
          style={{
        ...
        <span style={{ color: 'pink', fontSize: '19px' }}>
          <b>1 ETH: {this.state.exchangeRate} UNI</b>
        </span>
        <br></br>
        <span style={{ color: 'pink', fontSize: '17px' }}>
          How much ETH do want to use on this pathway?
        </span>
        <span>
          <input
            onChange={this.txAmtHandler}
            value={this.state.swapAmt}
          ...
          <>
            <button
              ...
              onClick={async () => {
                ...
                let data = await functionBook[this.props.functionLetter](
                  ...params,
                );
                this.setState({
                  transacted: true,
                ...
                {this.state.txHashes.map((hash, idx) => {
                  ...
                  return (
                    <p
                      ...
                      key={idx}
                    >
                      Tx {idx + 1}: <a href={url}>view tx on etherscan</a>
                    </p>
                  );
                })}
              </p>
